...
pub fn parse_input() -> Vec<(Vec<String>, Vec<String>)> {
    INPUT.lines().map(|line| {
        ...
        (left.trim().split(' ').map(|x| x.to_string()).collect(), right.trim().split(' ').map(|x| x.to_string()).collect())
    }).collect()
}
...
pub fn sol2() {
    let result = parse_input()
            ...
            for permutation in permutations() {
                ...
                for item in new_input {
                    match item.len() {
                        2 => {
                            // 1 = [2, 5]
                            if !item.contains(permutation[2]) 
        ...
        .map(|(permutation, input)| {
            ...
            input.iter().map(|item| find_digit(&permutation, item)).collect::<String>().parse::<i32>().unwrap()
        })
        .sum::<i32>();
    dbg!(result);
}
